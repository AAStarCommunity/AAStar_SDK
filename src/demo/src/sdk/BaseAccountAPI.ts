...
export abstract class BaseAccountAPI {
  ...
  abstract encodeBatchExecute (  data: [string[], BytesLike[]]): Promise<string> 
  /**
  ...
  async createUnsignedUserOp (info: TransactionDetailsForUserOp): Promise<UserOperationStruct> {
   ...
   // let paymasterAndData: string | undefined
    ...
    if (this.paymasterAPI != null) {
      ...
      const userOpForPm = {
        ...partialUserOp,
        preVerificationGas: preVerificationGas * 3
      }
      const response = await this.paymasterAPI.getPaymasterAndData(userOpForPm)
      partialUserOp.paymasterAndData = response.paymasterAndData;
      partialUserOp.preVerificationGas = response.preVerificationGas;
      partialUserOp.verificationGasLimit = response.verificationGasLimit;
      partialUserOp.callGasLimit = response.callGasLimit;
    }
  ...
  async getUserOpReceipt (userOpHash: string, timeout = 30000, interval = 5000): Promise<string | null> {
    ...
    while (Date.now() < endtime) {
      const events = await this.entryPointView.queryFilter(this.entryPointView.filters.UserOperationEvent(userOpHash),  Math.max(0, block.number - 100))
      if (events.length > 0) {
